...
\centering
\caption{Illustration of two-camera survey (here with both cameras on a single aircraft). Both cameras survey they grey strip, separated by a time difference $t$. This surveyed strip has half-width $w$. The fields of view of the cameras are indicated by dark grey strips spanning the width of the surveyed strip. It may be possible for animals within a wider strip of half-width $b$, to move into or out of the grey strip between the passing of the first and second camera. $Y_0$ is the distance to the right of the trackline (the dark black centre line), of an animal at the time it comes into the first camera's field of view and $Z_0$ is its availability status at this time ($Z_0=1$ if available, $Z_0=0$ if not).}
