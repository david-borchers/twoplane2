...
<<fig_boxplots_bcs, fig.env = "figure", fig.width = 6, fig.height= 4, fig.cap = "Box plots of estimated density for each of the 36 scenarios. The horizontal line is at true density \\(D=1.24\\). ", cache=TRUE>>=
...
caption = "Simulation results for LCE and Palm estimators from 1,000 simulations. Here gamma is the proportion of time animals are availabld, lag is the time between obsrvers, sigma is the animal diffusion rate parameter, mean(n) and mean(m) are the mean numbers of detections by one observer and the mean number of recaptures, across simulations.",
