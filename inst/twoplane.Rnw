...
We compare the LCE method with the recently-developed cluster capture-recapture method (CCR), which uses a Palm likelihood approximation, so providing the first comparison of CCR with maximum likelihood. Both methods are approximately unbiased with nominal confidence interval coverage and similar precision. The LCE estimator has slightly lower variance, more so as sample size increases. We illustrate the methods with semi-synthetic data from a harbor porpoise survey.
...
In almost all cases coverage probability is close to 95\% (Table~\ref{tab:mlesims_bcs}), with coverage probability tending to be worse for shorter lags and greater movement ($\sigma$). We conclude that for these scenarios, the LCE estimator is approximately unbiased, with confidence interval coverage usually close to nominal. The performance of the LCE and CCR estimators is very similar, with the LCE estimator making a slight gain in precision as sample size increases.
