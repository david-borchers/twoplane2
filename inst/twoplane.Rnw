...
\begin{abstract}
\todo[inline]{RMF - Abstract still needs to be checked and might need reangling}
  Line transect aerial surveys can be a relatively cheap way of surveying wildlife populations, and we anticipate that they will increasingly be carried out by unmanned aerial vehicles. Because aircraft move quickly, these surveys typically violate the conventional line transect assumption of certain detection on the line if animals are not continuously available for detection. In this case, mark-recapture distance sampling with two observers may be effective. Here we develop a mark-recapture method for surveys with two high-resolution digital cameras, which involves marginalizing over the latent capture histories that arise when animals cannot be individually identified but may move and change their availability between the passage of the two cameras. The survey transect is partitioned into small segments within which there is ambiguity over animal identity, and the likelihood is obtained by enumerating all possibilities within each segment. We describe the new method as `Latent Capture-history Enumeration', or LCE. The method requires  that the species of interest be identified on each camera, but does not require identification of ``recaptures'' on different cameras. %and that mean dive cycle length be specified: individual identification of animals is not required. In addition to estimating animal density, the method estimates the proportion of time animals are available for detection and the mean speed of animal movement.

We compare the LCE method with the recently-developed cluster capture-recapture (CCR) method, which tackles the same problem using a Palm likelihood approximation and does not involve a true likelihood. We apply both methods to the same data taken from a survey of harbour porpoise ({\em Phocoena phocoena}) in the North Sea. The LCE method is found to be approximately unbiased, to have close to nominal \todo{lets see...}confidence interval coverage, and to be slightly more precise than the CCR method. It is less able to cope with a very large number of latent capture histories than is the CCR method, but is more extendable in some respects.


% We apply the method to estimate porpoise density from the data in the paper by Stevenson, Borchers and Fewster (2018) ``Cluster capture-recapture to account for identification uncertainty on aerial surveys of animal populations'' in \textit{Biometrics}, and we investigate estimator properties by simulation.

\todo[inline]{RMF - added a bit more detail and LCE name. Removed CCR ref for a smoother read.}
...
Here, we develop an analysis framework suitable for two-camera aerial surveys, whether these are conducted from two aircraft, or from a single UAV carrying two cameras, which we anticipate will be a common survey mode in the future. We explicitly model animal movement into and out of the detection strip between the passage of the two cameras, corresponding to an ``in/out'' availability process that induces dependence between the two cameras. For diving animals, we further consider an ``up/down'' availability process by modeling the diving cycle. As noted by \cite{Stevenson+al:18}, two-observer survey data do not contain sufficient information to identify all parameters of the diving model, if the time delay between cameras is less than the mean dive-cycle duration. In that case, one parameter must be estimated from external data: we take this to be the mean dive-cycle duration itself. We derive our methods in generality including both in/out and up/down availability processes, but the methodology is equally applicable when only the in/out process is required, and in that case there is no need for external data.

To fit the two-camera model we use a full maximum-likelihood approach, which has not previously been achieved for data with uncertain capture histories, due to the computational intractability of enumerating all feasible matchings of sightings to animals. We assemble the likelihood by identifying sections of the transect line that have ambiguous animal identities, and enumerating all possible matchings only within these sections. As long as animal density is reasonably low, the enumeration is manageable within each section, and our approach is computationally feasible. We call our new method the ``Latent Capture history Enumeration'' (LCE) approach. \cite{Stevenson+al:18} derived an alternative approach to the same model using the new technique of cluster capture-recapture (CCR) \cite{Fewster+al:16}(Fewster et al., 2016), in which the locations of detections are treated as a clustered point-process and the model is fitted using a Palm likelihood approximation. In essence, the CCR approach fits an objective function to the distribution of pairwise distances between all pairs of detections. Although the CCR method is asymptotically consistent and computationally very efficient, it is not based on a true likelihood. While CCR has greater potential for extension to more complicated survey scenarios and high animal densities, it has not previously been possible to evaluate its performance against that of maximum likelihood, because likelihoods are not generally computable in the scenarios for which CCR is intended. Another key output of our work is therefore to create the first comparison between CCR and maximum likelihood.

Although our work is the first fully likelihood-based approach for aerial surveys to incorporate both availability models and uncertain capture histories, each of these has received substantial attention in its own right. Availability models together with double-observer designs are developed by \cite{Borchers+al:13}, \cite{Langrock+al:13}, and \cite{Borchers+Langrock:15}. Previous work on uncertain capture histories has focused on methods for resolving uncertainties before fitting conventional models. \cite{Pike:15}Pike (2015) used a logistic regression technique to decide on an optimal dissimilarity score between pairs of detections, then established a threshold score within which pairs would be resolved as duplicates. \cite{Hamilton+al:18}Hamilton et al.\ (2018) devised estimates of the duplicate probability for each pair of detections, and repeatedly resampled from these probabilities within a bootstrap scheme to create a new resolved dataset at each iteration. Other work on latent capture histories has focused on models where the observed capture histories are known corruptions of the true histories, but the truth is not recoverable (\cite{Link+al:10}Link et al., 2010). Such models have been fitted by likelihood approximation (\cite{Zhang+al:19}Zhang et al.\ (2019)) and by Bayesian MCMC methods (e.g.\ \cite{Bonner+Holmberg:13}Bonner and Holmberg 2013; \cite{McClintock:15}McClintock 2015). These models differ from our scenario here, because the data consist of discrete capture histories that are observed with certainty, despite not being resolvable back to the true histories. In our case, no capture histories are observed: only a set of detections from each aircraft in continuous time and space.
...
Recall that $X_{i1}$ and $X_{i2}$ are detections of animal $i$ by two observers separated by a time lag. With lags close to zero, $X_{i1}$ and $X_{i2}$ are highly correlated because animals available to one observer are almost certain to be available to the other. As lag increases, we expect this correlation to decrease. A pertinent question is whether dependence can be removed by choosing a suitably long lag. To investigate this we look at the correlation between $X_{i1}$ and $X_{i2}$ as a function of lag. We consider a range of $\gamma$ values from 0.1 to 0.9, and lags from 0 to $500$ seconds.
...
In practice, we are primarily interested in methods for surveys with two cameras on one aircraft, and with this configuration and fast-moving aircraft, lags of more than some tens of seconds are unlikely to be achievable. In light of this, and the results of Figure~\ref{fig:fig_correlation_plot}, we present simulations for (a) a scenario designed to imitate the porpoise survey above, and (b) scenarios with lag $l$ of 10, 20, 50 and 80 seconds, and $\gamma$ equal to 10\%, 20\%, 50\% and 80\%. We do this for $\sigma$ equal to 1.5 m/s \citep[the speed estimated by][]{Hiby+Lovell:98}, 0.95 m/s \citep[the speed estimated by][]{Westgate+al:95}, and 0.5 m/s \citep[a speed lower than that estimated above or by][]{Stevenson+al:18}. In all cases, we use the same $\tau$ in estimation as was used in simulation\todo{DLB: added this sentence}. For the short-lag scenarios in (b), we perform simulations with true density $D=1.24$, as estimated in the previous section, and with an observer speed of 100 knots, which is around the typical speed of marine aerial surveys.
...
The empirical bias of the LCE and CCR density estimators from the simulations are $\Sexpr{prt(bias.mle[1], 1)}$\% (CV=$\Sexpr{prt(cv.mle[1], 1)}$\%) and $\Sexpr{prt(bias.palm[1], 1)}$\% (CV=$\Sexpr{prt(cv.palm[1], 1)}$\%), respectively. The biases reduce to $\Sexpr{prt(bias.mle[2], 1)}$\% (CV=$\Sexpr{prt(cv.mle[2], 1)}$\%) and $\Sexpr{prt(bias.palm[2], 1)}$\% (CV=$\Sexpr{prt(cv.palm[2], 1)}$\%), respectively when sample size is doubled while holding density constant, and to $\Sexpr{prt(bias.mle[3], 1)}$\% (CV=$\Sexpr{prt(cv.mle[3], 1)}$\%) and $\Sexpr{prt(bias.palm[3], 1)}$\% (CV=$\Sexpr{prt(cv.palm[3], 1)}$\%) when sample size is trebled.
...
The correlation between LCE and CCR density estimates from the simulations is $\Sexpr{prt(correlation[1], 2)}$\todo{BCS: Changed from \Sexpr{round(simres$Dhat.cor,2)}}, while the probability of getting a relative difference as large as, or larger than, that observed is approximately $\Sexpr{prt(100*p.val[1], 0)}$\%\todo{BCS: Changed from $\Sexpr{round(pvalDdif,2)}$\%}, from which we conclude that the observed difference is not large enough to raise concerns about the validity of either estimator with the porpoise data.
...
\todo[inline]{RMF - Table 1 has numerous strange rounding errors for $\gamma$, e.g.\ 0.21 instead of 0.2, etc.
  Also the values of speed include 0.65 which differs from the values specified above. The $\gamma$ and lag columns only need 1 decimal place for $\gamma$ and don't need decimal places for lag.}

<<fig_boxplots_bcs, fig.env = "figure", fig.width = 6, fig.height= 4, fig.cap = "Box plots of estimated density for each of the 36 scenarios. The horizontal line is at true density \\(D=1.24\\). \\todo[inline]{BCS: Looks ugly because all of the `outliers' due to having 1000 iterations. Probably need to make this look nicer...}", cache=TRUE>>=
par(mar = c(5, 4, 0, 0), oma = rep(0.1, 4))
all.Ds <- sapply(short.res, function(x) x$mle$D.est)
plot.new()
plot.window(xlim = c(1, length(short.res)), ylim = c(0, max(all.Ds)))
for (i in 1:length(short.res)){
    boxplot(short.res[[i]]$mle$D.est, add = TRUE, at = i, axes = FALSE)
}
...
<<fig_boxplots, fig.env = "figure", fig.width = 6, fig.height= 4, fig.cap = "Box plots of estimated density for each of the 36 scenarios. The horizontal line is at true density \\(D=1.24\\). \\todo[inline]{BCS: OLD VERSION for comparison.}", cache=TRUE>>=
fns = readRDS("./results/filenames-D_1.24-FixedL-FixedN-simethod_MLE-Nsim_150.Rds")
...
simtab <- readRDS("./results/simtab-D_1.24-FixedL-FixedN-simethod_MLE-Nsim_150.Rds")
#simtab <- readRDS("./results/simtab-D_1.24-RandomL-FixedN-simethod_MLE-Nsim_150.Rds")
#mlesimtab = simtab[,c(2:4,7,9:11,13,21,23)]
mlesimtab = cbind(simtab[, c("gamma", "k", "sigmarate")], short.mle.bias, short.mle.cv, short.D.coverage, short.palm.bias, short.palm.cv, short.mean.n1, short.mean.m)
names(mlesimtab) = c("gamma","lag","sigma","\\%BiasLCE","\\%cvLCE","\\%CoverLCE","\\%BiasPalm","\\%cvPalm","mean(n)","mean(m)")
rownames(mlesimtab) <- NULL
mlesimtab$sigma = mlesimtab$sigma * 1000
print.xtable(xtable(mlesimtab,
caption = "Full sims with Nsim = 1000. The columns mean(n) and mean(m) are the averages from my simulations. Is that correct, or are they supposed to be calculated analytically?",
...
#simtab <- readRDS("./results/simtab-D_1.24-RandomL-FixedN-simethod_MLE-Nsim_150.Rds")
#mlesimtab = simtab[,c(2:4,7,9:11,13,21,23)]
mlesimtab = simtab[,c("gamma","k","sigmarate","pc.bias.mle","pc.cv.mle","cover.mle","pc.bias.palm","pc.cv.palm","n1","m")]
names(mlesimtab) = c("gamma","lag","sigma","\\%BiasLCE","\\%cvLCE","\\%CoverLCE","\\%BiasPalm","\\%cvPalm","mean(n)","mean(m)")
mlesimtab$sigma = mlesimtab$sigma * 1000
print.xtable(xtable(mlesimtab,
#caption = "Preliminary sims with Nsim=300.",
caption = "OLD VERSION: Preliminay sims with Nsim=150 (n,m are number obs by 1 observer, and number recaptures). The first column is scenario number.",
label = "tab:mlesims"),
sanitize.text.function=function(x){x},
caption.placement = "top",
size = "\\setlength{\\tabcolsep}{6pt}",
...
The LCE density estimator is unbiased or very nearly unbiased \todo[inline]{Need to update after lots of simulations. BCS: Still seems to be the case.} in all 36 scenarios. Figure~\ref{fig:fig_mlepalm_bias} shows the empirical bias as a function of the mean number of detections by each observer, together with the empirical bias of the CCR estimator fitted to the same simulated data. The bias of the two estimators is very similar. The correlation between the two estimators varies from $\Sexpr{prt(min(short.cor), 3)}$ to $\Sexpr{prt(max(short.cor), 3)}$\todo{BCS: Changed from 0.92--0.998.} across the 36 scenarios, and the mean difference of the estimator means from the true density, as a percentage of the true density, is $\Sexpr{prt(mean(short.mle.bias), 2)}\%$ in the case of the LCE estimator, and $\Sexpr{prt(mean(short.palm.bias, 2))}\%$\todo{BCS: Changed from 0.75 and -0.20\%} in the case of the CCR estimator.
...
The coefficients of variation of the LCE and CCR density estimators for all 36 scenarios are shown in Figure~\ref{fig:fig_mlepalm_cv}. The CVs decrease with sample size, as expected. The CV of the LCE estimator is smaller than that of the CCR estimator by a small percentage of the LCE estimator CV, with the percentage improvement due to the LCE method increasing as sample size increases\todo[inline]{BCS: No longer true in general. With the smallest sample sizes CCR is more precise.}. We interpret this to be a consequnce of the fact that the CCR estimator is not a maximum likelihood estimator, being based instead on an approximation to the Palm likelihood of pairwise comparisons between detections, so it does not have the asymptotic efficiency of a maximum likelihood estimator\todo[inline]{BCS: But I think this is still a sensible conclusion.}. Nevertheless, the difference in precision of the two estimators is very small, with the difference being no more than about 8\% relative to a CV of about 9\% for the largest sample size considered, which has about 270 detections by each observer\todo[inline]{BCS: I haven't changed these values because I was not sure how they were calculated. \\
  DLB: I think the sentence meant abs(CV$_{CCR}$-CV$_{LCE}$)/CV$_{LCE}$=0.08 for the largest sample size
scenario, and CV$_{LCE}$=0.09 for this scenario. That statement is consistent with the new
Figure 6, although strangely not with the old Figure 6. Maybe I had a preview of the new
Figure 6 when I wrote it? To simplify, we could end the sentence after "very small". Figure
6 tells the story better anyway.}.
...
<<fig_mlepalm_cv, fig.env = "figure", fig.width = 8, fig.height= 4, fig.cap = "Percentage coefficient of variation (\\%CV), as a function of mean number of detections by a single observer. The LCE estimator is represented by circles, the CCR estimator by crosses. Crosses are offset 8 units to the right, to avoid overlap with circles. Panel (a) shows the \\%CV. Panel (b) shows the amount by which the CV from the CCR method exceeds that from the LCE method, expressed as a percentage of the LCE CV.  \\todo[inline]{BCS: OLD VERSION for comparison.}", cache=TRUE>>=
par(mfrow=c(1,2))
x=1
...
Coverage probabilities at the 95\% level for the LCE estimator are shown in Table~\ref{tab:mlesims}. In all cases coverage probability is close to the nominal value.\todo[inline]{Coverage tends to be a bit high - suspend judgement until lots of simulations done.}

\todo[inline]{BCS: With the new simulation results, coverage looks pretty good! So I guess we don't have to bother thinking about all this :)}
...
This work was part-funded by the Royal Society of New Zealand through Marsden grant UOA-1418.
